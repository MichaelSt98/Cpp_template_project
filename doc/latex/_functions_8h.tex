...
\hypertarget{_functions_8h_autotoc_md102}{}\doxysubsection{Function parameters and arguments}\label{_functions_8h_autotoc_md102}
\hypertarget{_functions_8h_autotoc_md103}{}\doxysubsubsection{Pass by value}\label{_functions_8h_autotoc_md103}
By default, non-\/pointer arguments in C++ are passed by value. When an argument is {\bfseries{passed by value}}, the argument\textquotesingle{}s value is copied into the value of the corresponding function parameter. Therefore the original argument can not be modified by the function!\hypertarget{_functions_8h_autotoc_md104}{}\doxyparagraph{Pros}\label{_functions_8h_autotoc_md104}

\begin{DoxyItemize}
\item Arguments can be anything
\item Arguments are never changed by the function (prevents possibly unwanted side effects)
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md105}{}\doxyparagraph{Cons}\label{_functions_8h_autotoc_md105}

\begin{DoxyItemize}
\item Copying classes and structs can incur a significant performance penalty
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md106}{}\doxyparagraph{When to use}\label{_functions_8h_autotoc_md106}

\begin{DoxyItemize}
\item When passing fundamental data type and enumerators, and the function does not need to change the argument.
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md107}{}\doxyparagraph{When not to use}\label{_functions_8h_autotoc_md107}

\begin{DoxyItemize}
\item When passing structs or classes (including std\+::array, std\+::vector, and std\+::string)
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md108}{}\doxysubsubsection{Pass by reference}\label{_functions_8h_autotoc_md108}
...
Since functions can have only one return value, references or rather reference function parameters can be used to return multiple values.\hypertarget{_functions_8h_autotoc_md109}{}\doxyparagraph{read-\/only pass by reference}\label{_functions_8h_autotoc_md109}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md110}{}\doxyparagraph{Pros}\label{_functions_8h_autotoc_md110}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md111}{}\doxyparagraph{Cons}\label{_functions_8h_autotoc_md111}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md112}{}\doxyparagraph{When to use}\label{_functions_8h_autotoc_md112}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md113}{}\doxysubsubsection{Pass by address}\label{_functions_8h_autotoc_md113}
...
To change the address an argument points to, just pass the addresses by reference.\hypertarget{_functions_8h_autotoc_md114}{}\doxyparagraph{read-\/only pass by address}\label{_functions_8h_autotoc_md114}
If it is undesirable to change an argument, make it read-\/only by passing it as {\bfseries{const pointer}}, so that an error occurs if the function tries to change the argument.\hypertarget{_functions_8h_autotoc_md115}{}\doxyparagraph{Pros}\label{_functions_8h_autotoc_md115}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md116}{}\doxyparagraph{Cons}\label{_functions_8h_autotoc_md116}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md117}{}\doxyparagraph{When to use}\label{_functions_8h_autotoc_md117}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md118}{}\doxyparagraph{When not to use}\label{_functions_8h_autotoc_md118}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md119}{}\doxysubsubsection{Returning values by value, reference and address}\label{_functions_8h_autotoc_md119}
...
{\bfseries{Attention}}\+: Since local variables in a function go out of scope and are destroyed, this needs to be considered!\hypertarget{_functions_8h_autotoc_md120}{}\doxyparagraph{Return by value}\label{_functions_8h_autotoc_md120}
To use, when
\begin{DoxyItemize}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md121}{}\doxyparagraph{Return by address}\label{_functions_8h_autotoc_md121}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md122}{}\doxysubsubsection{Return by reference}\label{_functions_8h_autotoc_md122}
Not possible for literals or expressions (no address)!
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md123}{}\doxysubsubsection{Returning multiple values}\label{_functions_8h_autotoc_md123}
To return multiple values
\begin{DoxyItemize}
\item a struct (or in principle a class)
\item std\+::tuple can be used!
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md124}{}\doxysubsection{Function overloading}\label{_functions_8h_autotoc_md124}
...
{\bfseries{Attention\+:}} Function return types are not considered distinct!\hypertarget{_functions_8h_autotoc_md125}{}\doxysubsection{Default arguments}\label{_functions_8h_autotoc_md125}
A default argument is a default value provided for a function parameter. If the user does not supply an explicit argument for a parameter with a default argument, the default value will be used. If the user does supply an argument for the parameter, the user-\/supplied argument is used. Because the user can choose whether to supply a specific argument value, or use the default, a parameter with a default value provided is often called an optional parameter.\hypertarget{_functions_8h_autotoc_md126}{}\doxysubsection{Function pointers}\label{_functions_8h_autotoc_md126}
...
Using function pointers, functions can be passed to other functions as arguments.\hypertarget{_functions_8h_autotoc_md127}{}\doxysubsection{The stack and the heap}\label{_functions_8h_autotoc_md127}
...
\end{DoxyItemize}\hypertarget{_functions_8h_autotoc_md128}{}\doxysubsubsection{Heap}\label{_functions_8h_autotoc_md128}
The heap segment (also known as the “free store”) keeps track of memory used for dynamic memory allocation (when using {\itshape new} and {\itshape delete}).\hypertarget{_functions_8h_autotoc_md129}{}\doxysubsubsection{Stack}\label{_functions_8h_autotoc_md129}
The call stack (usually referred to as “the stack”) has a much more interesting role to play. The call stack keeps track of all the active functions (those that have been called but have not yet terminated) from the start of the program to the current point of execution, and handles allocation of all function parameters and local variables.\hypertarget{_functions_8h_autotoc_md130}{}\doxysubsection{Recursion}\label{_functions_8h_autotoc_md130}
Recursive functions are functions that call themselves.\hypertarget{_functions_8h_autotoc_md131}{}\doxysubsection{Variable number of parameters -\/ Ellipsis}\label{_functions_8h_autotoc_md131}
...
{\bfseries{Avoid using ellipsis, for many reasons ... !}}\hypertarget{_functions_8h_autotoc_md132}{}\doxysubsection{Lambdas -\/ anonymous functions}\label{_functions_8h_autotoc_md132}
...
{\bfseries{Use auto when initializing variables with lambdas, and std\+::function if you can’t initialize the variable with the lambda.}}\hypertarget{_functions_8h_autotoc_md133}{}\doxysubsection{Function templates}\label{_functions_8h_autotoc_md133}
